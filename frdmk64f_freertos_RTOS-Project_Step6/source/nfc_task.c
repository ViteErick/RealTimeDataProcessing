...
#define print_bufTest(x,y,z,v)  {int loop; PRINTF(x); for(loop=0;loop<z;loop++) PRINTF("%.2x ", y[loop]); v[loop] = y[loop] ; PRINTF("\n");}
...
void displayCardInfo(NxpNci_RfIntf_t RfIntf)
{
    ...
    case (MODE_POLL | TECH_PASSIVE_NFCA):
        ...
        int sizeNFCID = 0 ;
        sizeNFCID = (int)RfIntf.Info.NFC_APP.NfcIdLen;

        int NfcID_Read [7];

        for(loop=0;loop<RfIntf.Info.NFC_APP.NfcIdLen;loop++)
        {
        	NfcID_Read [loop] = (RfIntf.Info.NFC_APP.NfcId[loop]);
        ...
        if(addUser)
        {
        	...
        	xQueueSendToBack(dbAddUser_queue, NfcID_Read , 0); //move the servo
        }

        xQueueSendToBack(dbAuthenticate_queue, NfcID_Read , 0); //move the servo
    }
}
