...
#define dataBase_Authenticate_PRIORITY (configMAX_PRIORITIES - 3)
#define dataBase_AddUser_PRIORITY (configMAX_PRIORITIES - 3)

extern void database_task(void *pvParameters);
...
extern void dataBase_Authenticate(void *pvParameters);
extern void dataBase_AddUser(void *pvParameters);

extern void ftm_init(void);
extern void BOARD_InitGPIOInterrupts (void);

#define TASK_NFC_STACK_SIZE		800
...
QueueHandle_t dbAuthenticate_queue = NULL;
QueueHandle_t dbAddUser_queue = NULL;
...
int main(void)
{
	...
	/*	dataBase_Authenticate Queue*/
	dbAuthenticate_queue = xQueueCreate(10, 40);
    /*	dataBase_AddUser Queue*/
	dbAddUser_queue = xQueueCreate(10, 40);
	...
	if (xTaskCreate(keepalive_task, "KeepAlive_task", configMINIMAL_STACK_SIZE + 100, NULL, keepalive_task_PRIORITY, NULL) != pdPASS)
	{
    ...
    if (xTaskCreate(tcpipserver_task, "tcpipserver_task", configMINIMAL_STACK_SIZE + 300, NULL, tcpipserver_task_PRIORITY, NULL) != pdPASS)
	{
		PRINTF("Task creation failed!.\r\n");
		while (1)
			;
	...
	if (xTaskCreate(dataBase_Authenticate,
					"dataBase_Authenticate",
					configMINIMAL_STACK_SIZE + 300,
					NULL,
					dataBase_Authenticate_PRIORITY,
					NULL) != pdPASS)
	{
		PRINTF("Task dataBase_Authenticate creation failed!.\r\n");
		while (1)
			;
	}

    if (xTaskCreate(dataBase_AddUser,
					"dataBase_AddUser",
					configMINIMAL_STACK_SIZE + 300,
					NULL,
					dataBase_AddUser_PRIORITY,
					NULL) != pdPASS)
	{
		PRINTF("Task dataBase_Authenticate creation failed!.\r\n");
		while (1)
